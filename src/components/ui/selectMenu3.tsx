    ...
    <div>
      <div className="grid grid-cols-3">
        {data.map(item => (
          <div 
            key={item.id}
            className={`text-14 flex flex-col items-center p-1.5 gap-1.5 cursor-pointer ${tab === item.id ? 'text-white bg-travel-secondary100' : 'text-travel-gray400 bg-white'}`}
      ...
      {data.filter(item => tab === item.id).map(item => (
        <div key={item.id}>
          {item.description}
        </div>
      ))}
